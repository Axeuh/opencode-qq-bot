...
```
onebot_client.py (协调器，305行)
    │
    ├── ClientInitializer → client_initializer.py (310行)
    ├── HTTPCallbackHandler → http_callbacks.py (464行)
    ├── TaskExecutor → task_executor.py (106行)
    └── LifecycleManager → lifecycle_manager.py (196行)
```
...
| 模块 | 行数 | 关键类 | 职责 |
|------|------|--------|------|
| `onebot_client.py` | 305 | `OneBotClient` | 主协调器，组合所有组件 |
| `client_initializer.py` | 310 | `ClientInitializer` | 组件初始化，依赖注入 |
| `http_callbacks.py` | 464 | `HTTPCallbackHandler` | 12个HTTP回调处理函数 |
| `task_executor.py` | 106 | `TaskExecutor` | 定时任务执行逻辑 |
| `lifecycle_manager.py` | 196 | `LifecycleManager` | 热重载、重启、事件注册 |

### 💬 消息处理 (5个文件)
| 模块 | 行数 | 关键类 | 职责 |
|------|------|--------|------|
| `message_router.py` | 731 | `MessageRouter` | 消息路由决策器 |
| `command_system.py` | 906 | `CommandSystem` | 12个命令处理器 |
| `message_queue.py` | 243 | `MessageQueueProcessor` | 异步队列处理 |
| `api_sender.py` | 334 | `ApiSender` | API 消息发送器 |
| `file_handler.py` | 933 | `FileHandler` | 文件下载/三层回退 |

### 🔗 连接管理 (3个文件)
| 模块 | 行数 | 关键类 | 职责 |
|------|------|--------|------|
| `connection_manager.py` | 533 | `ConnectionManager` | WebSocket 连接生命周期 |
| `connection_lifecycle.py` | 283 | `ConnectionLifecycle` | 连接状态机 |
| `napcat_http_client.py` | 353 | `NapCatHttpClient` | HTTP API 客户端 |

### 🤖 OpenCode 集成 (3个文件)
| 模块 | 行数 | 关键类 | 职责 |
|------|------|--------|------|
| `opencode_integration.py` | 550 | `OpenCodeIntegration` | OpenCode 服务桥接层 |
| `opencode_api.py` | 312 | `OpenCodeAPI` | API 封装 |
| `opencode_initializer.py` | 142 | `OpenCodeInitializer` | 服务初始化 |

### 🔧 系统功能 (6个文件)
| 模块 | 行数 | 关键类 | 职责 |
|------|------|--------|------|
| `http_server.py` | 1197 | `HTTPServer` | HTTP API 服务器 |
| `restart_handler.py` | 234 | `RestartHandler` | 热重启处理器 |
| `config_manager.py` | 203 | `ConfigManager` | 配置管理/日志设置 |
| `task_scheduler.py` | 329 | `TaskScheduler` | 定时任务调度 |
| `task_storage.py` | 218 | `TaskStorage` | 任务持久化 |
